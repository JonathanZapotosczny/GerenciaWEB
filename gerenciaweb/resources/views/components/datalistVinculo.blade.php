                ...
                <tr>
                    <td class="d-none d-md-table-cell">{{ $item['nome'] }}</td>
                        <td class="d-none d-md-table-cell"> 
                            <select name="id_professor" class="form-select form-select-lg">
                            @foreach ($data[1] as $prof)
                                @foreach($data[0] as $vin)
                                    <option @if($vin->id == old('id_professor')) selected="true" @endif>
                                        {{ $vin->id_professor }}
                                    </option>
                                @endforeach
                            @endforeach
                    ...
                    <td>
                        ...
                        <a nohref style="cursor:pointer" onclick="showInfoModal('{{ $item['nome'] }}', '{{ $item[''] }}')" class="btn btn-outline-primary">
                    ...
                    <form action="{{ route('vinculos.destroy', $item['id']) }}" method="POST" id="form_{{$item['id']}}">
                        @csrf
                        @method('DELETE')
                    </form>
                </tr>
